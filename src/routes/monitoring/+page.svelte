<script lang="ts">
	...
	import RealTimeChartCard from './components/RealTimeChartCard.svelte';
	import MainMonitoringGridCard from './components/MainMonitoringGridCard.svelte';
	import { generateAbnormalData } from '$lib/components/grid/dummyGrid';
	...
	let toolStartInfo: { toolCode: string; startTime: number; value: number } | undefined = undefined;
	...
	function collectChartData(message: IRawsData) {
		...
		realtimeChartData.productNo = 'TEST';

		toolStartInfo = {
			toolCode: message.tcode,
			startTime: message.time,
			value: message.predict
		};
...
<div class="flex h-full w-full gap-5">
	<div class="h-full w-[32%]">
		<MainMonitoringGridCard
			IsAbnormalErrorEffect={IsRealtimeErrorEffect}
			data={productHistory}
			{operationInfo}
		></MainMonitoringGridCard>
	</div>
	...
	<div class="h-full w-[68%]">
		<RealTimeChartCard
			chartNames={['realTimeLoadSurface', 'realTimeLossSurface']}
			isErrorEffect={IsRealtimeErrorEffect}
			bind:alarmTimeInfo
			bind:toolStartInfo
			bind:chartTextData
			bind:realtimeChartData
		></RealTimeChartCard>
	</div>
</div>
